...
#         -> mae / rms / mel 세 가지를 모두 돌려서 비교표로 보여줌
# =========================

# F1 허용 손실폭. 예: BEST_VAL_F1 - GATING_F1_TOLERANCE 이상을 유지하는 후보 중
# gating 비율(연산 절감)이 최대인 threshold를 선택.
GATING_F1_TOLERANCE = 0.02
GATING_MIN_WHEEZE_F1 = best_val_f1_binary - GATING_F1_TOLERANCE
...
for energy_mode in ENERGY_MODES:
    ...
    # validation set에서 이 모드의 에너지 분포를 먼저 수집
    val_energies = collect_gating_energies(val_files, TRAIN_TXT_DIR, energy_mode)
    print(f"  {energy_mode} 에너지 분포 (val) | min={val_energies.min():.6f}, "
          f"median={np.median(val_energies):.6f}, max={val_energies.max():.6f}")

    # 하위 0~45 percentile을 1percentile 간격으로 촘촘하게 후보 생성
    # (게이팅은 '조용한' segment를 스킵하는 목적이므로 하위 구간만 촘촘하면 충분)
    threshold_candidates = make_percentile_thresholds(val_energies, max_percentile=45, step=1.0)
    ...
    best_th, search_results = search_best_input_gating_threshold(
        ...
        min_wheeze_f1=GATING_MIN_WHEEZE_F1,
        verbose=False,  # percentile 후보가 많아 로그가 길어지므로 요약만 출력
    )

    print(f"  선택된 threshold (validation 기준): {best_th:.6f}")
